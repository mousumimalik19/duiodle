...
export default function SketchMode({
  ...
  const handleModeChange = useCallback(
    (newMode: Mode) => {
      ...
      if (newMode === "structure") {
        ...
        // Export canvas and call Gemini
        const dataUrl = canvasRef.current?.getDataUrl?.("image/png");
        if (dataUrl) {
          const base64 = dataUrl.replace("data:image/png;base64,", "");
          analyzeSketch(base64, "image/png", designTheme, fidelityLevel);
        }
      }
    },
    [analyzeSketch, geminiKey, onOpenSettings, designTheme, fidelityLevel],
  );
  ...
  const showStructureResult = mode === "structure" && !analyzing;
            ...
            >
              ...
              <div className="sketch-panel-section">
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {(["high", "medium", "low"] as FidelityLevel[]).map((f) => (
                    <button
                      key={f}
                      type="button"
                      data-ocid={`sketch.fidelity.${f}.toggle`}
                      ...
                      style={{
                        ...
                        background:
                          fidelityLevel === f
                            ...
                            : "transparent",
                        color:
                          fidelityLevel === f ? "#fbff29" : panels.textMuted,
                        fontSize: "0.75rem",
                        fontFamily: "Karla, sans-serif",
                        cursor: "pointer",
